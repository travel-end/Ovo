...
interface ApiService {
     ...
     * n为一页?首
     */
    @GET("soso/fcgi-bin/client_search_cp?n=${Constant.SEARCH_SONG_PAGE_SIZE}&format=json")
    suspend fun searchMusic(@Query("w") searchContent:String, @Query("p") offSet:Int): SearchSong
     ...
     * @param offset 页数
     */
    @GET("soso/fcgi-bin/client_search_cp?n=${Constant.SEARCH_SONG_PAGE_SIZE}&format=json&t=8")
    suspend fun searchAlbum(@Query("w") searchContent:String,@Query("p") offSet:Int): AlAlbum
