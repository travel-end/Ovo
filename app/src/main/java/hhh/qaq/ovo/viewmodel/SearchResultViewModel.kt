 ...
 * @Description
 */
class SearchResultViewModel(app:Application):BaseResViewModel<SearchResultRepository>(app,SearchResultRepository()) {
}

class SearchMusicViewModel(app: Application):BaseResViewModel<SearchResultRepository>(app,SearchResultRepository()) {
    var pageOffset:Int = 1
    var keyText:String? = ""
    var mData = arrayListOf<MusicItemVM>()
    val mAdapter = MusicItemAdapter(R.layout.item_music,mData)
...
class SearchAlbumViewModel(app: Application):BaseResViewModel<SearchResultRepository>(app,SearchResultRepository()) {
