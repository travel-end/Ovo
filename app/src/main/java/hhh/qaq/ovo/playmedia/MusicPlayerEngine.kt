...
//            "缓存设置cacheSetting:$cacheSetting".log(TAG)
    ...
    override fun onError(mp: MediaPlayer?, what: Int, extra: Int): Boolean {
        when (what) {
            MediaPlayer.MEDIA_ERROR_UNKNOWN, MediaPlayer.MEDIA_ERROR_SERVER_DIED -> {
                ...
                val msg = mHandler?.obtainMessage(PlayConstant.TRACK_PLAY_ERROR, errorInfo)
                if (msg != null) {
    ...
    override fun onCompletion(mp: MediaPlayer?) {
        if (mp == mMediaPlayer) {
            mHandler?.sendEmptyMessage(PlayConstant.TRACK_WENT_TO_NEXT)
        } else {
            mService.get()?.mWakeLock?.acquire(30000)
            mHandler?.sendEmptyMessage(PlayConstant.TRACK_PLAY_ENDED)
            mHandler?.sendEmptyMessage(PlayConstant.RELEASE_WAKELOCK)
        }
    }
    ...
    override fun onPrepared(mp: MediaPlayer?) {
        if (mp != null) {
            ...
            if (!mIsPrepared) {
                mIsPrepared = true
                val msg = mHandler?.obtainMessage(PlayConstant.PLAYER_PREPARED)
                if (msg != null) {
                    mHandler?.sendMessage(msg)
                }
            }
