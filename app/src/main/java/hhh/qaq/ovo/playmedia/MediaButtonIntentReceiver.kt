...
import hhh.qaq.ovo.ui.main.MainActivity
...
class MediaButtonIntentReceiver : BroadcastReceiver() {
    companion object {
                ...
                when (msg.what) {
                    ...
                    MSG_HEADSET_DOUBLE_CLICK_TIMEOUT -> {
                        ...
                        command = when (clickCount) {
                            1 -> PlayConstant.CMD_TOGGLE_PAUSE
                            2 -> PlayConstant.CMD_NEXT
                            3 -> PlayConstant.CMD_PREVIOUS
                            else -> null
                        }
                        if (command != null) {
        ...
        fun startService(context: Context, command: String) {
            ...
            i.action = PlayConstant.SERVICE_CMD
            i.putExtra(PlayConstant.CMD_NAME, command)
            i.putExtra(PlayConstant.FROM_MEDIA_BUTTON, true)
        ...
        intent?.let { i ->
            ...
            if (intentAction == AudioManager.ACTION_AUDIO_BECOMING_NOISY) {
                if (isMusicServiceRunning(context)) {
                    ...
                    startIntent.action = PlayConstant.SERVICE_CMD
                    startIntent.putExtra(PlayConstant.CMD_NAME, PlayConstant.CMD_PAUSE)
                ...
                if (event != null) {
                    ...
                    when (keycode) {
                        KeyEvent.KEYCODE_MEDIA_STOP -> command = PlayConstant.CMD_STOP
                        KeyEvent.KEYCODE_HEADSETHOOK, KeyEvent.KEYCODE_MEDIA_PLAY_PAUSE -> command =
                            PlayConstant.CMD_TOGGLE_PAUSE
                        KeyEvent.KEYCODE_MEDIA_NEXT -> command = PlayConstant.CMD_NEXT
                        KeyEvent.KEYCODE_MEDIA_PREVIOUS -> command = PlayConstant.CMD_PREVIOUS
                        KeyEvent.KEYCODE_MEDIA_PAUSE -> command = PlayConstant.CMD_PAUSE
                        KeyEvent.KEYCODE_MEDIA_PLAY -> command = PlayConstant.CMD_PLAY
                        KeyEvent.KEYCODE_MEDIA_FAST_FORWARD -> command = PlayConstant.CMD_FORWARD
                        KeyEvent.KEYCODE_MEDIA_REWIND -> command = PlayConstant.CMD_REWIND
                    }
                    if (command != null) {
                        if (action == KeyEvent.ACTION_DOWN) {
                            if (mDown) {
                                if (PlayConstant.CMD_TOGGLE_PAUSE == command || PlayConstant.CMD_PLAY == command) {
